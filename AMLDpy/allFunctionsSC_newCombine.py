...
def filterPeak(xCar,xDate,xDir,xFilename, outFolder,whichpass = 0):
   ...
   #    'PERIOD5MIN'])
    ...
    gdf_tog = pd.merge(gdf_buff,pass_info_new,on = ['PEAK_NUM'])
   ...
   # gdfcop = gdf_tot.loc[:,['PEAK_NUM','geometry','min_read','numtimes','verified','pass','pk_LAT','pk_LON','pk_maxCH4_AB']].drop_duplicates()
    gdfcop = gdf_pass_pks.loc[:,['PEAK_NUM','geometry','min_read','numtimes','verified','pass','pk_LAT','pk_LON','pk_maxCH4_AB']].drop_duplicates()

    gdf_justloc = gdfcop.loc[:,['min_read','pk_LAT','pk_LON']].reset_index(drop=True)
   
    combinedOP = weightedLoc(gdfcop,'pk_LAT','pk_LON','min_read','pk_maxCH4_AB').loc[:,:].rename(columns = {'LAT':'pk_LAT','LON':'pk_LON'}).reset_index()
    other = gdfcop.loc[:,['min_read','numtimes']].reset_index(drop=True)
    combinedOP1 = pd.merge(combinedOP,other,on=['min_read'])
    geometry_temp = [Point(xy) for xy in zip(combinedOP1['pk_LON'], combinedOP1['pk_LAT'])]
    crs = {'init': 'epsg:4326'}
    gdf_OP = gpd.GeoDataFrame(combinedOP1, crs=crs, geometry=geometry_temp)
    gdf_OP = gdf_OP.to_crs(epsg=32610).copy()
    gdf_OP_reduced = gdf_OP.loc[:,['min_read','geometry','numtimes','pk_LON','pk_LAT']].drop_duplicates().reset_index(drop=True)
    gdf_OP_reduced.to_file(new_loc_json, driver="GeoJSON")
    
    gdf_OP_wrecombine = pd.merge(gdf_OP,gdf_pass_pks.loc[:,['pk_LON','pk_LAT','recombine']],on=['pk_LON','pk_LAT'])
    ...
    unique_peaks = gdf_pass_pks.loc[:,['PEAK_NUM','pk_LAT','pk_LON','min_read']].drop_duplicates()
