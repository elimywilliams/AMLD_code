...
raw_data_loc = "/Users/emilywilliams/Documents/GitHub/Trussville/test_august_files"

## Folder to put results in (will make subfolders later)
results_folder_loc = "/Users/emilywilliams/Documents/GitHub/Trussville/test_august_files/"

car_id = 'TrussAeris'  # CAR NAME TO APPEAR IN FILENAMES OBSERVED PEAK NAMES
...
from AMLD_server_functions import unique, unIfInt, \
...
to_filter_unique = list(np.unique(np.array(to_filter)))
...
if __name__ == '__main__':
    ...
    for index,file in enumerate(to_filter_unique):
...
if not os.path.exists(final_main_csv_loc):
    ...
    addingFiles = True
    toCombine = list(np.unique(np.array(list(map(lambda x: 'FilteredPeaks_' + x[6:],to_filter)))))
