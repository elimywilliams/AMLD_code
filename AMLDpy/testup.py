...
def ProcessRawDataAerisTxt(read_file, maxSpeed='45', minSpeed='0'):
    ...
    if (read_file.shape[0] != 0):
        ...
        sHeader = ['TIMESTAMP', 'INLET', 'PRESS_MBAR', 'TEMPC', 'CH4', 'H20', 'C2H6', 'R', 'C2C1', 'BATTV', 'POWMV',
                   'CURRMA', 'SOCPER', 'LAT', 'LONG',
        ...
        read_file['fdate'] = read_file.apply(
            lambda x: datetime(int(x.TIMESTAMP[6:10]), int(x.TIMESTAMP[0:2]), int(x.TIMESTAMP[3:5]),
                               int(x.TIMESTAMP[11:13]),
                               int(x.TIMESTAMP[14:16]), int(x.TIMESTAMP[17:19]),
                               int(float(x.TIMESTAMP[19:23]) * 1000000)), axis=1)
    ...
    try:
        ...
        for i in lstCH4_AB:
            if (cnt != 0):
                ...
                if ((float(aEpochTime[i]) - float(
                        aEpochTime[prevIndex])) > xTimeThreshold):  # initial start of a observed peak
                    cntPeak += 1
                    xTime = float(aEpochTime[i])
                    xDistPeak = 0.0
                    xCH4Peak = 0.0
